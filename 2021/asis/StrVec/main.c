...
{
  char name[0x10];
  readline("Name: ", name, sizeof(name));   // input1 - nope
  printf("Hello, %s!\n", name);

  int n = readint("n = ");    // input2
  ...
  while (1) {
    ...
    case 2: {
      int idx = readint("idx = ");   // input5
      char *data = (char*)malloc(0x20);
      if (!data)
        break;
      readline("data = ", data, 0x20);  // input6 - nope
